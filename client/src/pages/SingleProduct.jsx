...
function SingleProduct() {
  ...
  function addToCart(amount) {
    ...
    if (existingCartItem) {
      ...
      let quantity = 0;
      if (amount) {
        quantity = parseInt(existingCartItem.quantity) + parseInt(amount);
      } else {
  ...
  function StyleFeats({ style }) {
    if (style) {
      return (
        <div className="mb-4">
          <div className="text-green text-lg font-bold">${style.price}</div>
          ...
          <div className="m-5">
            <div className="card">
              <div className="card-body">
                <div className=" [color:#979291]">{product.description}</div>
            ...
            <div className="flex justify-center items-center flex-col [color:#979291]">
              <div className="m-5 text-left flex flex-wrap gap-4">
                {[]
                  ...
                  .sort((a, b) => a.height.value - b.height.value)
                  .map((style) => {
                    return (
                      <div key={style.name} className="radio">
                        <button
              ...
              {StyleFeats({
                style: product.styles.find((s) => s.name === selectedStyleName),
              })}
              <button
                onClick={() => {
                  addToCart(quantity);
